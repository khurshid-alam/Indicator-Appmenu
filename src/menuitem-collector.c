...
static void menuitem_collector_class_init (MenuitemCollectorClass *klass);
static void menuitem_collector_init       (MenuitemCollector *self);
...
static MenuitemCollectorFound * menuitem_collector_found_new (DbusmenuClient * client, DbusmenuMenuitem * item, GStrv strings, guint distance, GStrv usedstrings, const gchar * indicator_name);
...
tokens_to_children (DbusmenuMenuitem * rootitem, const gchar * search, GList * results, GStrv label_prefix, DbusmenuClient * client, const gchar * indicator_name)
{
	...
	if (!dbusmenu_menuitem_get_root(rootitem) && newstr != NULL) {
		...
		if (distance < G_MAXUINT) {
			...
			results = g_list_prepend(results, menuitem_collector_found_new(client, rootitem, newstr, distance, used_strings, indicator_name));
		}
...
static GList *
process_client (MenuitemCollector * collector, DbusmenuClient * client, const gchar * search, GList * results, const gchar * indicator_name, GStrv prefix)
{
	...
	if (search == NULL || search[0] == '\0') {
		...
		for (child = children; child != NULL; child = g_list_next(child)) {
			...
			results = g_list_prepend(results, menuitem_collector_found_new(client, item, (GStrv)array, calculate_distance(NULL, (GStrv)array, NULL), NULL, indicator_name));
		}
...
static GList *
just_do_it (MenuitemCollector * collector, const gchar * dbus_addr, const gchar * dbus_path, const gchar * search, GList * results, const gchar * indicator_name, GStrv prefix)
{
...
menuitem_collector_found_new (DbusmenuClient * client, DbusmenuMenuitem * item, GStrv strings, guint distance, GStrv usedstrings, const gchar * indicator_name)
{

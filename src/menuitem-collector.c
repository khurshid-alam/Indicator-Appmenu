...
static GList *
tokens_to_children (MenuitemCollector * collector, DbusmenuMenuitem * rootitem, const gchar * search, GList * results, GStrv label_prefix, DbusmenuClient * client)
{
	if (search == NULL) {
		return results;
	}
	...
	for (child = children; child != NULL; child = g_list_next(child)) {
		...
		results = tokens_to_children(collector, item, search, results, newstr, client);
	}
...
process_client (MenuitemCollector * collector, DbusmenuClient * client, const gchar * search, GList * results, GStrv prefix)
{
	...
	results = tokens_to_children(collector, dbusmenu_client_get_root(client), search, results, prefix, client);
	return results;
}
